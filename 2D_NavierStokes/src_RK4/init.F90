...
!
            CASE ("A")
               read(22) timein,((Z(ikx,iky+klag),ikx=1,kx1),iky=1,ky1)
!
            CASE ("B")  
     ...
     &                    ((Z(ikx,iky+klag),ikx=1,kx1),iky=1,ky1)
               ENDIF
!
            CASE ("C")
               read(22) timein,PERIODin,SHIFTXin,NERin,NER2,                    &
...
!
     ...
     &                    ((Z(ikx,iky+klag),ikx=1,kx1),iky=1,ky1)
            CASE ("J")        
               read(22) timein,dummy,PERIODin,SHIFTXin,SHIFTYin,NERin,dummy,                & 
     &                    ((Z(ikx,iky+klag),ikx=1,kx1),iky=1,ky1)
!
            CASE DEFAULT
...
!         TSTART = TIMEIN
         ...
         IF (irest.NE.0 .AND. WNFLAG.EQ.1) THEN
            print*,'Adding rescale factor ...',scale
         ENDIF
...
!
            ...
            ENDIF
!
           IF (irest.NE.0 .AND. WNFLAG.EQ.1 .AND. WK.GT.1.5_rk) THEN
            PHASE = RANNO(0)
!            if(KX==1.0 .and. KY==0.0) Z(IKX,IKY)=Z(IKX,IKY)* EXP(ZI*PHASE)
!            if(KX==0.0 .and. KY==1.0) Z(IKX,IKY)=Z(IKX,IKY)* EXP(ZI*PHASE)
!            Z(IKX,IKY) = Z(IKX,IKY)*L(IKX,IKY)*(1._rk*EXP(ZI*PHASE))
!            Z(IKX,IKY) = Z(IKX,IKY)*L(IKX,IKY)*(1._rk+scale*EXP(ZI*PHASE))
           ENDIF
!
