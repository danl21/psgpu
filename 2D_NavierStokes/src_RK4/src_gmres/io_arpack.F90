  ...
  implicit none
!
  integer(kind=4), parameter :: SzV = 22428  ! should be equal to 2*COUNT(LL)
!
!     %----------------------%
...
!
    ...
    DO IKY = 1, IKTY
       DO IKX = 1, IKTX
          ...
          q(IKX,IKY) = CMPLX(d(cnt),d(cnt+1),rk)
          cnt = cnt + 2
       ENDDO
    ENDDO
!
...
!
    ...
    DO IKY = 1, IKTY
       DO IKX = 1, IKTX
          IF(.NOT.LL(IKX,IKY))     CYCLE
          d(cnt) = REAL(v(IKX,IKY))
          d(cnt+1) = AIMAG(v(IKX,IKY))
          cnt = cnt + 2
       ENDDO
    ENDDO
!
