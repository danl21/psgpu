!
      SUBROUTINE INIT (XII,ETA,ZET,KX,KY,KZ,TSTART)
!
...
!
      ...
      REAL(rk)        :: KKX,KKY,KKZ,WK,EK,PK
      REAL(rk)        :: PHASE,ENSTRO,R1,R2,VK,timein
...
!
               ...
               close(22)
            CASE ("B")
               open(22,file=trim(infile),form='unformatted')
...
!            and randomising the phases
               ...
               IF ( WK.GT.1.5_rk .AND. WK.LE.3.5_rk .AND. KZ(IK).EQ.1 ) THEN
                  PK = AMPV         ! perturbation
               ELSE
                  PK = 0.0_rk
               ENDIF
                  
               IF ( IKX.EQ.2 .AND. KY(IK).EQ.1._rk .AND. KZ(IK).EQ.0._rk ) THEN
                  EK = 0.25_rk        ! taylor green vortex
               ELSEIF ( IKX.EQ.2 .AND. KY(IK).EQ.-1._rk .AND. KZ(IK).EQ.0._rk ) THEN
                  EK = -0.25_rk        ! taylor green vortex
               ELSE
                  EK = 0.0_rk
               ENDIF
               ...
               IF (irest .eq. 0 ) THEN
                  ! note this isn't necessarily div free, correction will happen at first step
!                  PHASE = RANNO(0)
!                 XII(IK) = PK*EXP(ZI*PHASE)
!                  PHASE = RANNO(0)
!                  ETA(IK) = PK*EXP(ZI*PHASE)
                  PHASE = RANNO(0)
                  ZET(IK) = EK + PK*EXP(ZI*PHASE)
               ENDIF
!
...
print*, 'initial enstrophy= ',ENSTRO
      !
! -------------------------------------------------------
!
      END SUBROUTINE INIT
